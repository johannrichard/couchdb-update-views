...
describe('Customer', function () {
	...
	it('should create new cusomer search index', function (done) {
		removeDesignDoc(db, function (err) {
			should.not.exist(err, 'error removing design document: ' + JSON.stringify(err, null, ' '))
			var customerIndex = { indexes: {
				editions: {
						index: function (doc) {
			...
			var docPath = '_design/customer_search'
			update(db, docPath, customerIndex, function (err, reply) {
				db.get(docPath, function (err, reply) {
					should.not.exist(err)
					should.exist(reply.indexes.editions)
					done()
				})
			})
		})
	...
	it('should update customer search index', function (done) {
		var customerIndex = { indexes: {
			editions: {
				index: function (doc) {
					index('default', doc.name);
		...
		update(db, docPath, customerIndex, function (err, reply) {
			...
			db.get(docPath, function (err, reply) {
				should.not.exist(err)
				should.exist(reply)
				var inputTestIndex = customerIndex.indexes.editions
				var inputTestViewIndex = inputTestIndex.index.toString().replace(/\s/g, '')
				var newViews = reply.indexes
				var newViewIndex = newViews.editions.index.toString().replace(/\s/g, '')
				newViewIndex.should.eql(inputTestViewIndex)
				done()
			})
